...
function App() {
  ...
  const [showGetPerson, setShowGetPerson] = useState(false);
  const contractAddress = "0x811cd7D8DAE4B9b839b5bdb39DE9490AbD15B649"; // Paste the address recieved from Remix IDE here
      ...
      <header className="App-header">
        ...
        : (
          <div>
            ...
            <button onClick={() => setShowGetPerson(!showGetPerson)}>{showGetPerson ? "Back" : "Name and Address"}</button>
            <button onClick={() => setUpdateName(!updateName)}>{updateName ? "Back" : "Update Name"}</button>
            {updateName && (
              <>
              <div>
              <input
